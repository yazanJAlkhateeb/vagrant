...
# start.bat/sh 127.0.0.1 40022 --> will call vagrant destroy , vagrant up , then deploy on local vagrant machine
...
# start.bat/sh --> then provide 127.0.0.1 and 40022 when prompted --> will call vagrant destroy , vagrant up , then deploy on local vagrant machine
...
scp -i ./deploy/id_rsa  -oStrictHostKeyChecking=no   -P $port ./database/create_user.sql  $user@$ip:temp
scp -i ./deploy/id_rsa  -oStrictHostKeyChecking=no   -P $port ./deploy/deploy.ps1 $user@$ip:
ssh -i ./deploy/id_rsa  -oStrictHostKeyChecking=no   -p $port $user@$ip  dos2unix deploy.ps1 temp/create_user.sql
